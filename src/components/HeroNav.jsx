...
function HeroNav() {
  ...
  // One-time refresh logic for perfect rendering
  useEffect(() => {
  ...
  useLayoutEffect(() => {
    const updateWidths = () => {
      setTimeout(() => {
        // Ensures text is fully rendered before measuring
        const newWidths = textRefs.current.map(
          (ref) => (ref ? ref.getBoundingClientRect().width + 30 : 60)
        );
        setWidths(newWidths);
